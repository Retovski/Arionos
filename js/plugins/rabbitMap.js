/*:
@plugindesc Displays a minimap based on pictures you have saved
@author poorrabbit
@version: 1.03
@target: MZ
@help
============================================================================
rabbitMap - A poorrabbit Games plugin
============================================================================
...
rabbitMap gives you a customizable mini map based on your images. 
...
Picture IDs:
 ...
 These picture IDs are to be used by rabbitMap to show the minimap pictures.
...
are using the Proxmity Compass, rabbitMap can be integrated nicely.
...
1.02 Fixed a bug in MV compatibility where rabbitMap would fail to draw
...
Spriteset_Map.prototype.update = function() {
    ...
    this.updateShadow();
    this.updateWeather();
 if (eval(rabbitMapMVCompat) == false) //Are we using MV? If so, don't do these.
	{
...
"use strict";
const parameters = PluginManager.parameters("rabbitMap");
...
const rabbitMapMVCompat = (parameters['MVCompat'] || false);

if (!rabbitNewMapID) {	var rabbitNewMapID = 0; }
if (!rabbitMapFile) { var rabbitMapFile = "";}
if (!rabbitMapX) { var rabbitMapX = "100";}
if (!rabbitMapY) { var rabbitMapY = "100";}
var rabbitMapRegX = /<rabbitMap: ([a-zA-Z0-9]+),([0-9]+),([0-9]+)>/i;
var isNewMap = 30;


    function rmGetMapInfo (){
        $gameScreen.erasePicture(mapPictureID);
        var rabbitMapNotes = $dataMap.note.match(rabbitMapRegX);
        if (rabbitMapNotes){
        rabbitMapFile = '../maps/' + rabbitMapNotes[1];
        rabbitMapX = rabbitMapNotes[2];
        rabbitMapY = rabbitMapNotes[3];
        }
        else { 
		...
		rabbitMapFile = '../maps/map%1'.format(rabbitNewMapID.padZero(3)); 
		rabbitMapX = 30;
        rabbitMapY = 30;
    ...
    function miniMap () {
        if (SceneManager._scene.constructor.name === "Scene_Map") {
			...
			var showingMiniMap = $gameSwitches.value(showMiniMapSwitch);
			//if we are on a new map, we show nothing for 30 frames.
			if ($gamePlayer.newMapId() > 0) {
				rabbitNewMapID = $gamePlayer.newMapId();
					isNewMap = 30;
			}
			if (!showingMiniMap) { isNewMap=30;}  //if the map has been turned off, and turned back on, we need a reset
			if (((rabbitNewMapID == $gameMap.mapId()) && (rabbitNewMapID > 0))) {
				rmGetMapInfo();
					isNewMap = 30;
				}
				
			if (rabbitMapFile == ""  && $gameMap.mapId() > 0) {
					rabbitNewMapID = $gameMap.mapId();
					rmGetMapInfo();
						isNewMap = 30;
			};
			...
			try { //in MV it seems to take a couple of extra frames to catch up, so rather than crash, we try/catch out.
					if (showingMiniMap)  {	//are we showing the minimap?
						if ($gamePlayer.newMapId() == 0) { //is this a new map? if NOT, do the thing.
						let tw = (rabbitMapX /100) || 1; //set the X scale based on the Map Notetag (unless it's not set, then 1)
						let th = (rabbitMapY /100) || 1; //set the Y scale based on the Map Notetag (unless it's not set, then 1)
						...
						$gameScreen.showPicture(mapBGPictureID, "../maps/" + backgroundFile, 0, eval(mapBGXOffset) + eval(xBase) -(mw/2) , eval(mapBGYOffset) + eval(yBase) -(mh/2) , 100, 100, 255, 0); 
						
						if (isNewMap > 0) //if we're on a new map - show nothing for 30 frames (see above)
							{
							$gameScreen.showPicture(mapPictureID, rabbitMapFile, 0, eval(xBase) - xr * $gamePlayer.x -5 , eval(yBase) - yr * $gamePlayer.y -3 , eval(rabbitMapX), eval(rabbitMapY), 0, 0);
							isNewMap -= 1;
						}
						else {
							if (mapScrollSpeed) {  //if the map scrollspeed is set, it "moves" the map to it's next position over that many frames
								$gameScreen.movePicture(mapPictureID, 0, eval(xBase) - xr * $gamePlayer.x -5, eval(yBase) - yr * $gamePlayer.y -3, eval(rabbitMapX), eval(rabbitMapY), 255, 0, mapScrollSpeed);
							}
							else { //otherwise it just plops it where it needs to be
								$gameScreen.showPicture(mapPictureID, rabbitMapFile, 0, eval(xBase) - xr * $gamePlayer.x -5, eval(yBase) - yr * $gamePlayer.y - 3 , eval(rabbitMapX), eval(rabbitMapY), 255, 0);
							}
								
						}
						//this mask file determines the "hole" where the minimap is actually displayed
						$gameScreen.showPicture(mapMaskPictureID, "../maps/" + maskFile, 0, eval(xBase) - (mw/2)  , eval(yBase) - (mh/2), 100, 100, 255, 0); 
												
						//Are we showing the player dot?
						if ( eval(isShowPlayerDot) == "true" || eval(isShowPlayerDot) > 0 ){
						$gameScreen.showPicture(mapPlayerDotPictureID, "../maps/" + playerDotFile , 0, eval(xBase) + eval(pDXO) -5 , eval(yBase) + eval(pDYO) -3 , 100, 100, 255, 0); 
						}
                ...
                if (!showingMiniMap) { //if not, delete the picture and mask
						$gameScreen.erasePicture(mapPictureID);
